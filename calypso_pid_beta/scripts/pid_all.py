...
class pid_gypseas:
  ...
  def __init__(self):
    ...
    self.m = interp1d([0, 90],[1581,2000])
    self.n = interp1d([-90, 0], [1500,1581])
  ...
  def gypseas_pid(self):
    ...
    # fir testing purpose
    PID_roll = PID.getPID(self.roll,False)
    ...
    self.g=gypseas()
    self.g.t1 = round(PID_heave + PID_roll)
    self.g.t2 = round(PID_heave - PID_roll)
    # self.g.t3 = round(PID_heave - PID_roll + PID_pitch)
    # self.g.t4 = round(PID_heave + PID_roll + PID_pitch)

    print("PID-roll : ", PID_roll, " PID-heave : ", PID_heave)
  ...
  def Imu_subscriber(self, Imu):
    ...
    print("current_position : ", 3 - self.heave.current_position)
    self.gypseas_pid()
  ...
  def get_current_pose(self, Imu):
    self.heave_temp = Imu.z
